...
class Config():
    def __init__(self,  
        ...
        self.output_dir = f'ckpt/SalientSleepNet/sleepedf-{self.subjects_num}/fold{self.fold_id}_subject{self.subject}/{self.model.init_time}'

def main(fold_id,chs,subjects_num):
    print(f"------------------------------------FOLD_{fold_id}------------------------------------")
    cfg = Config(fold_id=fold_id,subject=subjects[fold_id],SS=5,subjects_num=subjects_num)
