...
from util.train_test import test_3ch, test_4ch, test_mul, train, test, train_3ch, train_4ch, train_mul,train_salient,test_salient
...
class Config():
    def __init__(self,  
        ...
        self.loss_fn=nn.CrossEntropyLoss(weight=torch.Tensor([1.0, 1.80, 1.0, 1.25, 1.20]).to(device))
        self.scheduler = None
        ...
        self.output_dir = f'ckpt/SalientSleepNet/isruc-sleep-{self.subjects_num}/fold{self.fold_id}_subject{self.subject}/{self.model.init_time}'

def main(fold_id,chs,subjects_num):
    print(f"------------------------------------FOLD_{fold_id}------------------------------------")
    cfg = Config(fold_id=fold_id,subject=subjects[fold_id],SS=10,subjects_num=subjects_num)
    ...
    for t in range(cfg.epochs):
        ...
        if chs==3:
            ...
            train_loss, train_acc = train_salient(train_loader, net, loss_fn, optimizer)
            test_loss,test_acc = test_salient(test_loader, net, loss_fn)
...
if __name__ == '__main__':
    ...
    if "shhs" in args2.np_data_dir:
        folds_data,subjects = load_folds_data_shhs(args2.np_data_dir, 20)
        chs=2
        subjects_num=5000
        ...
        subjects_num=20
    elif "isruc" in args2.np_data_dir:
