...
from util.train_test import test_3ch, test_4ch, test_mul, train, test, train_3ch, train_4ch, train_mul,train_1ch,test_1ch
...
class Config():
    def __init__(self,  
        ...
        self.epochs = 700
        # Model
        self.model=model.AttnSleep_isruc().to(device)
        ...
        self.output_dir = f'ckpt/attnsleep/isruc-sleep-{self.subjects_num}-O2_A1/fold{self.fold_id}_subject{self.subject}/{self.model.init_time}'

def main(fold_id,chs,subjects_num):
    print(f"------------------------------------FOLD_{fold_id}------------------------------------")
    cfg = Config(fold_id=fold_id,subject=subjects[fold_id],SS=9,subjects_num=subjects_num)
