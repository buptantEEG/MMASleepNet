...
from util.train_test import test_3ch, test_4ch, test_mul, test_printnet, train, test, train_3ch, train_4ch, train_mul,train_1ch,test_1ch, train_printnet
...
class Config():
    def __init__(self,  
        ...
        self.epochs = 150
        # Model
        self.model=model.SleepPrintNet_isruc().to(device)
        ...
        self.output_dir = f'ckpt/SleepPrintNet/isruc-sleep-{self.subjects_num}/fold{self.fold_id}_subject{self.subject}/{self.model.init_time}'

def main(fold_id,chs,subjects_num):
    print(f"------------------------------------FOLD_{fold_id}------------------------------------")
    cfg = Config(fold_id=fold_id,subject=subjects[fold_id],SS=11,subjects_num=subjects_num)
    ...
    for t in range(cfg.epochs):
        ...
        elif chs == 1:#attnsleep
            train_loss, train_acc = train_1ch(train_loader, net, loss_fn, optimizer)
            test_loss,test_acc = test_1ch(test_loader, net, loss_fn)
        elif chs == 5: #printnet
            train_loss, train_acc = train_printnet(train_loader, net, loss_fn, optimizer)
            test_loss,test_acc = test_printnet(test_loader, net, loss_fn)
...
if __name__ == '__main__':
    ...
    if "isruc" in args2.np_data_dir:
        folds_data,subjects = load_folds_data_isruc(args2.np_data_dir, 10)
        subjects_num=10
