...
from util.train_test import test_3ch, test_4ch, test_mul, train, test, train_3ch, train_4ch, train_mul,train_salient,test_salient,train_1ch,test_1ch
...
class Config():
    def __init__(self,  
        ...
        self.output_dir = f'ckpt/attnsleep/sleepedf-{self.subjects_num}/fold{self.fold_id}_subject{self.subject}/{self.model.init_time}'

def main(fold_id,chs,subjects_num):
    print(f"------------------------------------FOLD_{fold_id}------------------------------------")
    cfg = Config(fold_id=fold_id,subject=subjects[fold_id],SS=1,subjects_num=subjects_num)
