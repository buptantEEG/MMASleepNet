...
class Temporal_feature_EEG(nn.Module):
    def __init__(self,inplanes):
        ...
        self.features1 = nn.Sequential(
            nn.Conv1d(2, 64, kernel_size=50, stride=8, bias=False, padding=24),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(), 
        ...
        self.features2 = nn.Sequential(
            nn.Conv1d(2, 64, kernel_size=500, stride=64, bias=False, padding=24),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(), 
...
class MMASleepNet_eegeog(BasicModel):
    def __init__(self,cfg):#,d_model,afr_reduced_cnn_size):
        super(MMASleepNet_eegeog, self).__init__()
        ...
        self.linear1 = nn.Sequential(nn.Linear(self.d_model*64,32),nn.ReLU(True))
    ...
    def forward(self, x1,x2,x3):
        ...
        x_cat=torch.cat((x_EEG,x_EOG),dim=1) #[256,4,64,24]
        # print("x_cat_2d.shape:",x_cat_2d.shape)
        x_afr = self.AFR(x_cat) #[256,3,64,24]
        # print(x_cat.shape)
        x_afr= x_afr.view(self.batch_size,-1,72) 
        # x_cat=x_cat.squeeze()
        # print(x_afr.shape)
        encoded_features=self.transformer_encoder(x_afr.view(self.batch_size,-1,72) )
